...
void loop() {
  ...
  if (stringComplete) {
    if (inputString.charAt(0) == 'j' || inputString.charAt(0) == 'J') {
      char idx = inputString.charAt(1);

      if (isDigit(idx)) {
        char w_idx = inputString.indexOf(' ');
        Jd[idx - 49] = inputString.substring(w_idx + 1, inputString.length()).toInt();
        // Serial.print("Joint ");
        // Serial.print(idx);
        // Serial.print(" ");
        // Serial.println(inputString.substring(w_idx+1,inputString.length()).toInt());
        print_angles();
      }
    }

    else if (inputString.charAt(0) == 'r' || inputString.charAt(0) == 'R') {
      reset_angles();
    } else if (inputString.charAt(0) == 'p' || inputString.charAt(0) == 'P') {
      print_angles();
    }
    ...
    B1Limiter.setTargetAndVelLimitForTimedMove(Jd[0], max(abs(Jd[0] - Jc[0]) / 15.0, 2));
    J1Limiter.setTargetAndVelLimitForTimedMove(Jd[1], max(abs(Jd[1] - Jc[1]) / 15.0, 2));
    J2Limiter.setTargetAndVelLimitForTimedMove(Jd[2], max(abs(Jd[2] - Jc[2]) / 15.0, 2));
    J3Limiter.setTargetAndVelLimitForTimedMove(Jd[3], max(abs(Jd[3] - Jc[3]) / 15.0, 2));
    J4Limiter.setTargetAndVelLimitForTimedMove(Jd[4], max(abs(Jd[4] - Jc[4]) / 30.0, 2));
  ...
  B1Limiter.calc();
  B1Setpoint = B1Limiter.getPosition();
  B1PID.Compute();
  B1_changeSpeed();

  J1Limiter.calc();
  J1Setpoint = J1Limiter.getPosition();
  J1PID.Compute();
  J1_changeSpeed();

  J2Limiter.calc();
  J2Setpoint = J2Limiter.getPosition();
  J2PID.Compute();
  J2_changeSpeed();

  J3Limiter.calc();
  J3Setpoint = J3Limiter.getPosition();
  J3PID.Compute();
  J3_changeSpeed();

  J4Limiter.calc();
  J4Setpoint = J4Limiter.getPosition();
  J4PID.Compute();
  J4_changeSpeed();
  ...
  // print_angles();
  // Serial.print("Desired : ");
  // Serial.print(J1Setpoint);
  // Serial.print(" ");
  // Serial.print("Actual : ");
  // Serial.println(J1Input);
}
